...
public static class LuaDefinitionGenerator
{
	...
	private static void AppendClass(StringBuilder builder, ScriptClass c)
	{
		...
		foreach (ScriptProperty p in c.Properties)
		{
			...
			builder.AppendLine($"{p.Name}: {p.Type.Luaify()}");
		}
		...
		foreach (ScriptEvent e in c.Events)
		{
			...
			{
				builder.Append($"<{string.Join(", ", e.Parameters.Select(p => p.Type.Luaify()))}>");
			}
			builder.AppendLine();
		}
		...
		foreach (ScriptMethod m in c.Methods)
		{
			if (m.IsMetamethod)
			{
				...
				if (m.IsStatic && (m.Parameters.Length == 0 || m.Parameters[0].Type.Name != c.Name)) continue;
				...
				{
					indexerIndexes.Add(m.Parameters[m.IsStatic ? 1 : 0].Type.Luaify());
					indexerValues.Add((isNewIndex ? m.Parameters[m.IsStatic ? 2 : 1].Type : m.Returns.FirstOrDefault(ScriptType.Nil)).Luaify());
					hasIndexer = true;
					continue;
				}
			}
			...
			IEnumerable<string> iter = m.Parameters.Select(p => p.LuaifyForFuncDef());
			if ((m.IsMetamethod && m.IsStatic) || m.IsSemiStatic)
			{
				// overwrite first parameter with self
				iter = iter.Skip(1);
			}
			builder.AppendLine($"\tfunction {m.Name}({string.Join(", ", iter.Prepend("self"))}): ({string.Join(", ", m.Returns.Select(t => t.Luaify()))})");
		}
	...
	private static void AppendStaticClass(StringBuilder builder, ScriptClass c)
	{
		...
		foreach (IGrouping<string, string> g in c.Methods
			...
			.GroupBy(
				m => m.Name,
				m => $"({string.Join(", ", m.Parameters.Select(p => p.LuaifyForFuncType()))}) -> ({string.Join(", ", m.Returns.Select(t => t.Luaify()))})"
			))
		{
