...
public class LuaMetatable : LuaObject
{
	...
	public virtual int Index(IntPtr L)
	{
		...
		if (prop != null)
		{
			...
			if (accessData.ObsoleteAttribute != null)
			{
				PT.PrintWarn(accessData.ObsoleteAttribute.GetWarning(prop.Name));
			}
	...
	private int ProcessMethod(LuaState state, Script script, ScriptService.MethodsCacheData methodInfos, object? targetObject, string key, object?[] args)
	{
		...
		if (invocationData.ObsoleteAttribute != null)
		{
			PT.PrintWarn(invocationData.ObsoleteAttribute.GetWarning(targetMethod.Name));
		}
